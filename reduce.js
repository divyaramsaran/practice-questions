...
const longestWord = function (words) {
  return words.reduce(function (longWord, word) {
    return word.length > longWord.length ? word : longWord;
  }, "");
};

console.log(longestWord(["apple", "banana", "cherry", "kiwi", "ramsaran"]));
//------------------------------------------------------------------------------

// shortestWord(["apple", "banana", "cherry", "kiwi"]) => "kiwi"
const shortestWord = function (words) {
  return words.reduce(function (shortestWord, word) {
    return word.length > shortestWord.length ? shortestWord : word;
  });
};
...
const joinWithComma = function (words) {
  return words.reduce((joinedWord, word) => joinedWord.concat(",", word));
};
...
const joinWordsWithSpace = (words) => words.join(" ");
console.log(joinWordsWithSpace(["apple", "banana", "cherry"]));
//------------------------------------------------------------------------------

// concatenateNames(["John", "Jane", "Doe"]) => "JohnJaneDoe"
const concatenateNames = (names) => names.join("");
console.log(concatenateNames(["John", "Jane", "Doe"]));
//------------------------------------------------------------------------------

// countVowelsInWords(["hello", "world"]) => "eoo"

const vowelArray = (vowels, letter) => {
...
const countVowelsInWords = (words) =>
  [...words.join("")].reduce(vowelArray, []).join("");
...
const makeCamelCase = (words) =>
  words.reduce((camelCase, word) => camelCase.push());
...
const findMinMax = function (numbers) {
  return numbers.reduce((minAndMax, number) => {
    minAndMax["min"] = minAndMax["min"] < number ? minAndMax["min"] : number;
    minAndMax["max"] = minAndMax["max"] > number ? minAndMax["max"] : number;
    return minAndMax;
  }, {});
};
...
const sumByCategory = function (items) {
  return items.reduce((sum, item) => {
    sum[item.category] = sum[item.category] || 0;
    sum[item.category] += item.value;
