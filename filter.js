...
const filterUsersByHashtagAndLikes = function (users, hashtag, minLikes) {}; //incomplete
...
const filterStringsByLengthAndMembership = function (
  ...
  return strings.filter(
    (string) => criteria.includes(string) && string.length > length
  );
};
...
const filterObjectsByProperties = function (objects, properties) {
  return objects.filter((object) =>
    Object.keys(object).includes(properties.at(0))
  );
};
...
// Output: ["hello", "hell"]
const filterStringsBySubstringMembership = function (strings, criteria) {}; //incomplete
...
const filterEvenNumbersByMembership = function (numbers, criteria) {
  const numbersPresentInBothArrays = numbers.filter((number) =>
    criteria.includes(number)
  );

  return numbersPresentInBothArrays.filter((number) => (number & 1) === 0);
};
...
console.log(
  findCountriesThatExist(["India", "USA", "Iran"], {
    India: "exists",
    USA: "does not exist",
    Iran: "exists",
  })
...
console.log(
  findActiveUsers(["Alice", "Bob", "Charlie"], {
    Alice: { status: "active" },
    Bob: { status: "inactive" },
...
console.log(
  findStringsAboveThreshold(["apple", "banana", "kiwi"], {
    apple: 4,
    banana: 5,
    kiwi: 6,
  })
...
console.log(
  findLongTermEmployees(["John", "Alice", "Bob"], {
...
const findLargeCities = function (cities, lookup) {
  return cities.filter((city) => lookup[city].population > 3000000);
};

console.log(
  findLargeCities(["London", "Paris", "Tokyo"], {
...
console.log(
  findInStockItems(["item1", "item2", "item3"], {
    item1: { quantity: 15 },
    item2: { quantity: 5 },
    item3: { quantity: 20 },
  })
...
const findAnimalsByHabitat = function (animals, animalHabitats, lookup) {
  return animals.filter((animal) => animalHabitats[animal].habitat === lookup);
};
